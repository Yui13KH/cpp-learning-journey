        ...
        for (int j = 65; j <= 90; j++) {
            for (int k = 65; k <= 90; k++) {
                ...
                if (passwordCheck == password) {
                    ...
                    cout << "Found after " << (i - 65) * 26 * 26 + (j - 65) * 26 + (k - 65) + 1
                         << " Trials" << endl;
                    return;
                } else {
                    cout << "Trial[" << (i - 65) * 26 * 26 + (j - 65) * 26 + (k - 65) + 1
                         << "]: " << passwordCheck << endl;
                }
            }
        }
...
// (i - 65) * 26 * 26 + (j - 65) * 26 + (k - 65) + 1 , doing this to calculate the entire number of trials 